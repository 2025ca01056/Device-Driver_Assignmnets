...
/usr/src/linux-headers-$(uname -r)/scripts/checkpatch.pl --file --no-tree core.c stats.c bitsfeed.c

echo "=== Step 2: Attempt Loading Consumer without Core ==="
sudo insmod ./bitsfeed.ko || echo "Expected failure: Unresolved symbols!"
...
sudo rmmod bitscore || echo "Expected failure: Module is in use!"
...
echo "=== All Lab Question 1 Tests Passed Successfully! ==="
